...
app.py
=======
Streamlit UI for the Staff Arabia Timesheet Generator.

Run locally with:
    streamlit run app.py

All the actual calculation logic lives in timesheet_engine.py — this file
is only responsible for the screen: uploading files, showing progress, and
offering the finished Excel file for download.
"""
...
st.set_page_config(
    page_title="Staff Arabia Timesheet Generator",
    page_icon="🗓️",
    layout="centered",
)
...
# ── Sidebar: advanced settings (optional, defaults match the original script) ─
with st.sidebar:
    ...
    selected_weekend_days = st.multiselect("Days treated as weekend", day_names, default=default_selected)
    ...
    if len(uploaded_files) != 3:
        ...
        if st.button("🚀 Generate Timesheet", type="primary", use_container_width=True):
            ...
            try:
                ...
                # Save workbook to an in-memory buffer for download
                buffer = io.BytesIO()
                wb.save(buffer)
                buffer.seek(0)
